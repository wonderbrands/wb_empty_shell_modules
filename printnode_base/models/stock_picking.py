...
class StockPicking(models.Model):
    ...
    shplbl_printed = fields.Boolean(string='Etiqueta impresa',default=False)
    shplbl_print_date = fields.Datetime(string='Fecha de impresion de etiqueta')
    ...
    def print_last_shipping_label(self):
        ...
        if self.picking_type_code != 'internal' and self.picking_type_code != 'outgoing':
            return
        #Campo para verificar la impreison de la guia por auxiliar
        utc_now = fields.Datetime.now()
        self.shplbl_print_date = utc_now
    ...
    @api.model
    def run_automatic_block_ship_label(self):
        utc_now = fields.Datetime.now()
        one_hour = timedelta(hours=1)
        stock_move_valpick = self.env['stock.picking'].search(
            [('shplbl_print_date', '!=', False), ('shplbl_printed', '=', False)])

        for rec in stock_move_valpick:
            shplbl_print_date = fields.Datetime.from_string(rec.shplbl_print_date)
            # shplbl_print_date -= timedelta(hours=6)# Hora servidor es igual a utc
            time_difference = utc_now - shplbl_print_date
            if time_difference >= one_hour:
                rec.write({'shplbl_printed': True})
            else:
                pass
    ...
    def _print_sl_from_attachment(self, raise_exception=True):
        ...
        attatchments = self.env['ir.attachment'].search(
        ...
        # domain.append(('create_date', '=', attatchments.create_date))
